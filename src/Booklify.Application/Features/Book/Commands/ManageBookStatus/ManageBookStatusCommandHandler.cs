    ...
    public async Task<Result<BookResponse>> Handle(ManageBookStatusCommand command, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (request.ApprovalStatus.HasValue && request.ApprovalStatus != existingBook.ApprovalStatus)
            {
                ...
                _logger.LogInformation("Admin {UserId} changed approval status of book {BookId} to {ApprovalStatus}", 
                    currentUserId, command.BookId, request.ApprovalStatus.Value);
            }
            ...
            if (!string.IsNullOrEmpty(request.ApprovalNote) && request.ApprovalNote != existingBook.ApprovalNote)
            {
                ...
                _logger.LogInformation("Admin {UserId} updated approval note for book {BookId}", 
                    currentUserId, command.BookId);
            }
            ...
            if (request.IsPremium.HasValue && request.IsPremium != existingBook.IsPremium)
            {
                ...
                _logger.LogInformation("Admin {UserId} changed premium status of book {BookId} to {IsPremium}", 
                    currentUserId, command.BookId, request.IsPremium.Value);
            }
